...
function jrtt_openbox() {
  ...
  const llUrl1 = {
    url: "https://is-lq.snssdk.com/score_task/v1/task/open_treasure_box/"
        + jrtt_sleepurlck,
    headers: {"sdk-version": 2, "x-Tt-Token": jrtt_sleepck}
  };

  $iosrule.post(llUrl1, function (error, response, data) {
    var obj = JSON.parse(data)
    if (log == 1) {
      console.log("今日头条开宝箱data:" + data);
    }
    if (obj.err_no == 0) {
      var tmm = obj.data.next_treasure_time - obj.data.current_time;
      result2 = "[金币]" + obj.data.score_amount + "[距下次开宝箱时间]" + formatSeconds(
          tmm);
...
function jrtt_matchbox(ssr) {
  ...
  const llUrl1 = {
    url: "https://is-lq.snssdk.com/score_task/v1/tips/get_data/?"
        + jrtt_sleepurlck,
    headers: {"sdk-version": 2, "x-Tt-Token": jrtt_sleepck}
  };
...
function jrtt_sleep_history(res) {
  ...
  const llUrl1 = {
    url: "https://i-lq.snssdk.com/score_task/v1/sleep/history/"
        + jrtt_sleepurlck,
    headers: {"sdk-version": 2, "x-Tt-Token": jrtt_sleepck}
  };
  ...
  $iosrule.get(llUrl1, function (error, response, data) {
    var obj = JSON.parse(data)
    if (log == 1) {
      console.log("今日头条碎觉查询data:" + data);
    }
    if (obj.err_no == 0) {
      result2 = "[总金币]" + obj.data[0].sleep_score_amount + "-"
          + obj.data[0].date + res;
...
function jrtt_sleep_done() {
  ...
  const llUrl1 = {
    url: "https://i-lq.snssdk.com/score_task/v1/sleep/done_task/"
        + jrtt_sleepurlck,
    headers: {
      "sdk-version": 2,
      "Content-Type": "application/json; encoding=utf-8",
      "x-Tt-Token": jrtt_sleepck
    },
    body: jrtt_sleepbd
  };
  ...
  $iosrule.post(llUrl1, function (error, response, data) {
    ...
    if (log == 1) {
      console.log("今日头条睡觉收割金币data:" + data);
    }
    if (obj.err_no == 0) {
...
function jrtt_sleep_mm() {
  ...
  var tt = jinritoutiao;
  const llUrl1 = {
    url: "https://i-lq.snssdk.com/score_task/v1/sleep/status/"
        + jrtt_sleepurlck,
    headers: {"sdk-version": 2, "x-Tt-Token": jrtt_sleepck}
  };
  ...
  $iosrule.get(llUrl1, function (error, response, data) {
    var obj = JSON.parse(data)
    if (log == 1) {
      console.log("今日头条睡觉状态查询data:" + data);
    }
    if (obj.err_no == 0) {
...
function jrtt_sleep_begin() {
  ...
  var tt = jinritoutiao;
  const llUrl1 = {
    url: "https://i-lq.snssdk.com/score_task/v1/sleep/start/?"
        + jrtt_sleepurlck,
    headers: {"sdk-version": 2, "x-Tt-Token": jrtt_sleepck},
    body: jrtt_sleepbd
  };
...
function jrtt_sign() {
  ...
  var tt = jinritoutiao;
  const llUrl1 = {
    url: "https://is-lq.snssdk.com/score_task/v1/task/sign_in/"
        + jrtt_sleepurlck,
    headers: {"sdk-version": 2, "x-Tt-Token": jrtt_sleepck}
  };
  const llUrl2 = {
    url: "https://is-lq.snssdk.com/score_task/v1/task/sign_in/detail/"
        + jrtt_sleepurlck,
    headers: {"sdk-version": 2, "x-Tt-Token": jrtt_sleepck}
  };
...
formatSeconds(value) {
  ...
  let h = Math.floor(result / 3600) < 10 ? '0' + Math.floor(result / 3600)
      : Math.floor(result / 3600);
  let m = Math.floor((result / 60 % 60)) < 10 ? '0' + Math.floor(
      (result / 60 % 60)) : Math.floor((result / 60 % 60));
  let s = Math.floor((result % 60)) < 10 ? '0' + Math.floor((result % 60))
...
function cotime(timestamp) {
  ...
  const m = (date.getMinutes() + 1 < 10 ? '0' + (date.getMinutes() + 1)
      : date.getMinutes() + 1) + ''
  return M + D + h + m
}
