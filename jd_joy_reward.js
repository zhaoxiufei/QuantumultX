...
const DualKey = '';
//如需双账号签到,此处单引号内填写抓取的"账号3"Cookie, 否则请勿填写
const ThreeKey = '';
//=======node.js使用说明结束=======
//直接用NobyDa的jd cookie
let cookie = Key ? Key : $.getdata('magic_cookie_1');
const cookie2 = DualKey ? DualKey : $.getdata('magic_cookie_2');
const cookie3 = ThreeKey ? ThreeKey : $.getdata('magic_cookie_3');
let UserName = '';
...
async function joyReward(doubleKey) {
  ...
  if (getExchangeRewardsRes.success) {
    ...
    console.log(
        `可兑换阶级 ${canPetLevel === 1 ? 'L1-5等级' : canPetLevel === 2 ? 'L6-10等级'
            : canPetLevel === 3 ? 'L10-15等级' : canPetLevel === 4 ? 'L6-20等级'
                : canPetLevel === 5 ? 'L21-25等级' : canPetLevel === 6
                    ? 'L26-30等级' : '最高级'}\n`);
    console.log(`京东昵称 ${pin}\n`);

    let todayExchanged = datas.some(
        (value, index, array) => value.todayExchanged === true)
    ...
    if (!todayExchanged) {
      for (let i = 0; i < canPetLevel; i++) {
        for (let j = 0; j < datas[i].rewardDetailVOS.length; j++) {
          if (datas[i].rewardDetailVOS[j].rewardType === 3
              && datas[i].rewardDetailVOS[j].petScore < score) {
      ...
      if (canReardArr && canReardArr.length > 0) {
        for (let item of canReardArr) {
          ...
          let levelArea = item.petLevel === 1 ? 'L1-5等级' : item.petLevel === 2
              ? 'L6-10等级' : item.petLevel === 3 ? 'L10-15等级' : item.petLevel
              === 4 ? 'L6-20等级' : item.petLevel === 5 ? 'L21-25等级'
                  : item.petLevel === 6 ? 'L26-30等级' : '最高级';
          if (item.leftStock > 0) {
            let exchangeRes = await exchange(item.id);
            if (exchangeRes.success) {
              if (exchangeRes.data === 'exchange_success') {
                console.log(`【京东账号${doubleKey ? '二'
                    : '一'}】${UserName}成功兑换 ${item.rewardName}花费 ${item.petScore}积分`);
                $.msg($.name, `成功兑换 ${item.rewardName}`, `【京东账号${doubleKey ? '二'
                ...
                console.log(
                    `兑换${levelArea}区域的【${item.rewardName}】失败，原因：今日兑换机会已用完`)
              }
            } else {
  ...
  } else if (!getExchangeRewardsRes.success && getExchangeRewardsRes.errorCode
      === 'B0001') {
    $.msg($.name, `【提示】京东账号${doubleKey ? '二' : '一'}cookie已失效,请重新登录获取`,
        '请点击此处去获取Cookie\n https://bean.m.jd.com/ \n',
  ...
  return new class {
    ...
    writedata() {
      if (this.isNode()) {
        ...
        e ? this.fs.writeFileSync(t, o) : i ? this.fs.writeFileSync(s, o)
    ...
    lodash_set(t, s, e) {
      return Object(t) !== t ? t : (Array.isArray(s) || (s = s.toString().match(
          /[^.[\]]+/g) || []), s.slice(0, -1).reduce(
          (t, e, i) => Object(t[e]) === t[e] ? t[e] : t[e] = Math.abs(s[i + 1])
          >> 0 == +s[i + 1] ? [] : {}, t)[s[s.length - 1]] = e, t)
    }
    ...
    setval(t, s) {
      return this.isSurge() || this.isLoon() ? $persistentStore.write(t, s)
          : this.isQuanX() ? $prefs.setValueForKey(t, s) : this.isNode()
    ...
    get(t, s = (() => {
      ...
      || this.isLoon() ? $httpClient.get(t, (t, e, i) => {
      ...
      }, t => s(t)) : this.isNode() && (this.initGotEnv(t), this.got(t).on(
          "redirect", (t, s) => {
            try {
              const e = t.headers["set-cookie"].map(
    ...
    post(t, s = (() => {
      ...
      && (t.headers["Content-Type"] = "application/x-www-form-urlencoded"), delete t.headers["Content-Length"], this.isSurge()
      || this.isLoon()) {
    ...
    time(t) {
      ...
      for (let e in s) {
        new RegExp("(" + e + ")").test(t) && (t = t.replace(
            RegExp.$1, 1 == RegExp.$1.length ? s[e] : ("00" + s[e]).substr(
                ("" + s[e]).length)));
      }
      return t
    }
    ...
    msg(s = t, e = "", i = "", o) {
      const h = t => !t || !this.isLoon() && this.isSurge() ? t : "string"
      == typeof t ? this.isLoon() ? t : this.isQuanX() ? {"open-url": t}
          : void 0 : "object" == typeof t && (t["open-url"] || t["media-url"])
          ? this.isLoon() ? t["open-url"] : this.isQuanX() ? t : void 0
          : void 0;
      $.isMute || (this.isSurge() || this.isLoon() ? $notification.post(s, e, i,
          h(o)) : this.isQuanX() && $notify(s, e, i, h(o))), this.logs.push("",
          "==============\ud83d\udce3\u7cfb\u7edf\u901a\u77e5\ud83d\udce3=============="), this.logs.push(
          s), e && this.logs.push(e), i && this.logs.push(i)
    }

    log(...t) {
      t.length > 0 ? this.logs = [...this.logs, ...t] : console.log(
          this.logs.join(this.logSeparator))
    }

    logErr(t, s) {
      const e = !this.isSurge() && !this.isQuanX() && !this.isLoon();
      e ? $.log("", `\u2757\ufe0f${this.name}, \u9519\u8bef!`, t.stack) : $.log(
          "", `\u2757\ufe0f${this.name}, \u9519\u8bef!`, t)
    }
    ...
    wait(t) {
      ...
      this.log("",
          `\ud83d\udd14${this.name}, \u7ed3\u675f! \ud83d\udd5b ${e} \u79d2`), this.log(), (this.isSurge()
